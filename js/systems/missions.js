...
export function initMissions(){
  on("email:read",({emailId})=>{
    for(const m of MISSIONS){
      if(m.startWhen.type==="email_read" && m.startWhen.target===emailId) startMission(m.id);
    }
  });
...
function recordObjective(type,target){
  ...
  for(const id of [...s.missions.active]){
    ...
    const obj=m.objectives.find(o=>o.type===type && o.target===target);
    if(!obj) continue;
    s.missions.progress[id] ??= {};
    s.missions.progress[id][obj.id]=true;
    emit("mission:progress",{mission:m,objective:obj});
    const done=m.objectives.every(o=>s.missions.progress[id]?.[o.id]);
    if(done) completeMission(id);
  }
}
...
function completeMission(id){
  ...
  addCredits(m.rewards?.credits||0);
  s.player.reputation+=(m.rewards?.reputation||0);
...
export function missionView(){
  ...
  return s.missions.active.map(id=>{
    const m=mission(id);
    return {...m,progress:s.missions.progress[id]||{}};
  });
}
