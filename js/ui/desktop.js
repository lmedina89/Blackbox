...
export function initDesktopUI({enterBlackbox}){
  ...
  function renderWindow(id,el){
    ({browser:renderBrowser,mail:renderMail,chat:renderChat,files:renderSystem,missions:renderMissions,servicedesk:renderServiceDesk,notes:renderNotes,threatdesk:renderThreatDesk}[id])?.(el);
  }
  function openApp(id){
  ...
  function renderServiceDesk(el){
    ...
    if(serviceDeskView.mode==="remote"&&snap.remoteSession){
      ...
      const remoteToolPane=()=>{
        ...
        if(tool==="access"){
          if(!machine.access)return `<div class="empty-state">No delegated identity controls are available on this workstation.</div>`;
          const groupSet=new Set(machine.access.groups||[]),shares=Object.entries(machine.access.shares||{});
          return `<div class="remote-access remote-tool-page"><h2>Users & Groups — ${escapeHtml(machine.hostname)}</h2><section class="classic-group"><legend>NEXUS\\${escapeHtml(machine.user.username)} memberships</legend><div class="access-core-groups">${(machine.access.groups||[]).filter(group=>!machine.access.editableGroups?.includes(group)).map(group=>`<span>${escapeHtml(group)}</span>`).join("")||"<span>None</span>"}</div><div class="access-group-list">${(machine.access.editableGroups||[]).map(group=>{const member=groupSet.has(group);return `<div class="access-group-row"><div><b>${escapeHtml(group)}</b><small>${group==="ENG-PROJECT-R"?"Read-only Engineering Projects access":group==="ENG-PROJECT-RW"?"Read/write Engineering Projects access":"Broad domain administrative authority"}</small></div><span class="access-state ${member?"member":"not-member"}">${member?"MEMBER":"NOT MEMBER"}</span><button class="classic-button" data-remote-group="${escapeHtml(group)}" data-enable="${member?"0":"1"}">${member?"REMOVE":"ADD"}</button></div>`;}).join("")}</div></section><section class="classic-group"><legend>Effective share access</legend>${shares.map(([id,share])=>{const allowed=(share.allowedGroups||[]).some(group=>groupSet.has(group));return `<div class="access-share-row"><div><b>${escapeHtml(share.name)}</b><small>${escapeHtml(share.path)}</small></div><span class="access-state ${allowed?"member":"not-member"}">${allowed?"READABLE":"ACCESS DENIED"}</span><small>Least privilege: ${escapeHtml(share.leastPrivilegeGroup||"Scoped group")}</small></div>`;}).join("")}</section><div class="cert-map"><b>Support rule:</b> restore only the access required for the user's stated job function. Broad administrator membership is not a substitute for the correct resource group.</div></div>`;
        }
    ...
    el.querySelector("[data-sd-remote]")?.addEventListener("click",()=>{const result=connectRemote(selected.id);toast(result.message,{titleText:"REMOTE ASSISTANCE"});if(result.ok){serviceDeskView.mode="remote";serviceDeskView.remoteTool="desktop";serviceDeskView.commandOutput="Type HELP for supported remote diagnostics.";}rerender();});
